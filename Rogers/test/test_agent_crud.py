...
def set_api_key(token: str, api_key: str):
    ...
    resp = requests.put(f"{BASE_URL}/api/agent/api-key",  # 修复路径: PUT /api/agent/api-key
...
def create_session(token: str, name: str = "测试会话"):
    """创建会话（前端生成 UUID，无需后端 API）"""
    import uuid
    session_id = str(uuid.uuid4())
    print(f"🆕 会话 ID 已生成: {session_id}")
    return session_id
...
if __name__ == "__main__":
    ...
    # 查询历史记录
    print("\n" + "="*50)
    print("📋 查询 Pipeline 历史记录")
    print("="*50)
    user_id = 1  # 测试账号 user_id=1
    hist_resp = requests.get(
        f"{BASE_URL}/api/agent/pipeline/history",
        params={"user_id": user_id, "session_id": session_id},
        headers={"Authorization": f"Bearer {token}"},
    )
    print(f"📨 历史记录: {json.dumps(hist_resp.json(), ensure_ascii=False, indent=2)}")
    ...
    print(f"📨 {json.dumps(delete_resp.json(), ensure_ascii=False, indent=2)}")
