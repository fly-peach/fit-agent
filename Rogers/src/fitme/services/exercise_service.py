...
class ExerciseService:
    ...
    - user_db: UserPinnedExercise (用户收藏)
    """
    ...
    def list_exercises(
        ...
        query = base_db.query(Exercise).filter(Exercise.is_active == True)
    ...
    @staticmethod
    def get_exercise(base_db: Session, user_db: Session, user_id: int, exercise_id: int) -> Optional[tuple]:
        """获取单个动作详情，附带收藏状态"""
    ...
    @staticmethod
    def get_target_muscles(base_db: Session) -> List[str]:
        """获取所有目标肌肉分类"""
        results = base_db.query(distinct(Exercise.target_muscle)).order_by(Exercise.target_muscle).all()
        return [r[0] for r in results if r[0]]

    @staticmethod
    def get_exercise_types(base_db: Session) -> List[str]:
        """获取所有动作类型"""
        results = base_db.query(distinct(Exercise.exercise_type)).order_by(Exercise.exercise_type).all()
        return [r[0] for r in results if r[0]]

    @staticmethod
    def get_equipment_list(base_db: Session) -> List[str]:
        """获取所有器械分类"""
        results = base_db.query(distinct(Exercise.equipment)).order_by(Exercise.equipment).all()
        return [r[0] for r in results if r[0]]
    ...
    @staticmethod
    def pin_exercise(base_db: Session, user_db: Session, user_id: int, exercise_id: int) -> Optional[UserPinnedExercise]:
        """收藏动作，如果已收藏则返回 None"""
        ...
        # 检查动作是否存在
        exercise = base_db.query(Exercise).filter(Exercise.exercise_id == exercise_id).first()
        if not exercise:
            return None
    ...
    @staticmethod
    def get_pinned_exercises(base_db: Session, user_db: Session, user_id: int) -> List[UserPinnedExercise]:
        """获取用户收藏列表（按 sort_order 排序）"""
        # 注意：由于 Exercise 在不同数据库，无法使用 joinedload
        # 需要先获取 UserPinnedExercise，再从 base_db 批量获取 Exercise
        ...
        if pinned_list:
            ...
            exercises = base_db.query(Exercise).filter(
