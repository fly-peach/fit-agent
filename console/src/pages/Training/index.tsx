...
const Training: React.FC = () => {
  ...
  const handleCreatePlan = async () => {
    try {
      ...
      await trainingApi.createPlan({ ...values, estimatedDuration: values.estimatedDuration, scheduledDate: values.scheduledDate.format('YYYY-MM-DD') })
  ...
  const handleCompletePlan = async () => {
    ...
    try {
      ...
      await trainingApi.completePlan(selectedPlanId, { ...values, completedDate: values.completedDate?.format('YYYY-MM-DD') })
  ...
  return (
    <div className="fitagent-page-enter" style={{ padding: isMobile ? 12 : 24 }}>
          ...
          {weekDays.map((day, idx) => {
            ...
            return (
              <div key={day} style={{ textAlign: 'center' }}>
                ...
                <br />
                <Avatar size={isMobile ? 32 : 40} style={{ backgroundColor: completed ? '#10B981' : '#d9d9d9', marginTop: 8, fontSize: isMobile ? 12 : 14 }}>
      ...
      <Card style={{ marginTop: isMobile ? 16 : 24, border: 'none' }}>
        ...
        <List itemLayout="horizontal" dataSource={schedule} renderItem={(item) => (
          <List.Item actions={item.status === 'completed' ? [] : [
            ...
            <List.Item.Meta
              avatar={<Avatar style={{ backgroundColor: item.status === 'completed' ? '#10B981' : '#0EA5E9' }}>{item.status === 'completed' ? '✓' : item.dayOfWeek}</Avatar>}
              title={item.planName}
      ...
      <Card style={{ marginTop: isMobile ? 16 : 24, border: 'none' }}>
        ...
        <Row gutter={[isMobile ? 12 : 16, isMobile ? 12 : 16]} style={{ marginTop: 8 }}>
          {recommendations.map(item => (
            <Col xs={24} sm={12} md={6} key={item.recommendId}>
              <Card size="small" className="fitagent-card-hover" style={{ border: 'none' }}>
                ...
                <div style={{ marginTop: 8 }}><Space>{getTypeTag(item.planType)}{getIntensityTag(item.intensity)}</Space></div>
                <Typography.Text type="secondary" style={{ fontSize: 12 }}>{item.duration}分钟</Typography.Text>
                {item.caloriesBurn && <Typography.Text type="secondary" style={{ fontSize: 12, display: 'block' }}>消耗 {item.caloriesBurn} kcal</Typography.Text>}
              </Card>
            </Col>
          ))}
        </Row>
      ...
      <Modal title="创建训练计划" open={createOpen} onCancel={() => setCreateOpen(false)} onOk={handleCreatePlan} okText="创建" cancelText="取消" width={isMobile ? '100%' : undefined} style={isMobile ? { top: 0, margin: 0, maxWidth: '100%' } : undefined}>
      ...
      <Modal title="完成训练" open={completeOpen} onCancel={() => setCompleteOpen(false)} onOk={handleCompletePlan} okText="提交" cancelText="取消" width={isMobile ? '100%' : undefined} style={isMobile ? { top: 0, margin: 0, maxWidth: '100%' } : undefined}>
        <Form form={completeForm} layout="vertical" style={{ marginTop: 16 }} initialValues={{ completedDate: dayjs() }}>
          ...
          <Form.Item name="actualDuration" label="实际时长" rules={[{ required: true }]}><InputNumber addonAfter="分钟" style={{ width: '100%' }} defaultValue={60} /></Form.Item>
          <Form.Item name="actualIntensity" label="实际强度" rules={[{ required: true }]}><Select options={intensities} /></Form.Item>
          <Form.Item name="caloriesBurned" label="消耗热量"><InputNumber addonAfter="kcal" style={{ width: '100%' }} defaultValue={0} /></Form.Item>
        </Form>
      </Modal>
    </div>
  )
